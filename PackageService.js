  ...
  process.nextTick(function() {
    if (resources && Array.isArray(resources) && resources.length > 0) {
      ...
      for (var i in resources) {
        ...
        Resource.findOne({ resourceName: _resource.resourceName }, function(
          err,
          resource
        ) {
          count++;
...
function GetPackage(req, res) {
  ...
  var jsonString;
  VPackage.findOne({ packageName: req.params.packageName }, function(
    err,
    vPackage
  ) {
    if (err) {
  ...
  VPackage.findOne({ packageName: req.params.packageName }, function(
    ...
    if (err) {
      ...
      if (vPackage) {
        ...
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "Delete Package Failed, package object is null",
          false,
          undefined
        );
        res.end(jsonString);
...
function UpdatePackage(req, res) {
  ...
  VPackage.findOneAndUpdate(
    { packageName: req.params.packageName },
    req.body,
    function(err, vPackage) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
...
function UpdatePackageUnit(req, res) {
  ...
  req.body.updated_at = Date.now();
  PackageUnit.findOneAndUpdate(
    { unitName: req.params.unitName },
    req.body,
    function(err, packageUnit) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Update Package Unit Failed",
          false,
          undefined
        );
      } else {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Update Package Unit Successful",
          true,
          packageUnit
        );
      }
...
function DeletePackageUnit(req, res) {
  ...
  var jsonString;
  PackageUnit.findOne({ unitName: req.params.unitName }, function(
    err,
    packageUnit
  ) {
    if (err) {
      ...
      if (packageUnit) {
        packageUnit.remove(function(err) {
          if (err) {
            ...
            jsonString = messageFormatter.FormatMessage(
              undefined,
              "Package Unit successfully deleted",
              true,
              undefined
            );
          }
        ...
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "Delete Package Failed, Package Unit object is null",
          false,
          undefined
        );
        res.end(jsonString);
...
function UpdateCodec(req, res) {
  ...
  Codec.findOneAndUpdate({ codec: req.params.codec }, req.body, function(
    err,
    codecObj
  ) {
    if (err) {
  ...
  Codec.findOne({ codec: req.params.codec }, function(err, codecObj) {
    if (err) {
      ...
      if (codecObj) {
        ...
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "Delete Codec, Codec object is null",
          false,
          undefined
        );
        res.end(jsonString);
...
function GetCodecByType(req, res) {
  ...
  var jsonString;
  PackageUnit.find({ type: req.params.type, active: true }, function(
    err,
    codecObj
  ) {
    if (err) {
